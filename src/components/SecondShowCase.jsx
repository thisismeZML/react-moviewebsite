...
const SecondShowCase = () => {
  ...
  const fetchMovie = async () => {
    try {
      ...
      if (res.data.results.length === 0) {
        ...
        setShowCaseMovie(res.data.results[5]); // Set the first movie from the now-playing list
      }
    } catch (error) {
  ...
  return (
    <div className="relative h-full w-full">
      ...
      />
      <div className="container absolute inset-0 flex items-center justify-end text-white z-30">
      ...
      <div className="absolute inset-0 flex items-center justify-start text-white z-20 bg-black opacity-20"></div>
    </div>
  );
};
